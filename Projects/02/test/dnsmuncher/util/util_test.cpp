...
TEST(ByteTest, MultipleWidthBitset) {
	...
	EXPECT_EQ(6, (unsigned long) convert_to_char_big_endian<3>( t1, 1 ));
	EXPECT_EQ(4, (unsigned long) convert_to_char_big_endian<3>( t1, 0));

	std::bitset<16> t2(std::string("0101110110000110"));
	EXPECT_EQ(93, (unsigned long) convert_to_char_big_endian<8>( t2, 1));
	EXPECT_EQ(134, (unsigned long) convert_to_char_big_endian<8>( t2, 0));
