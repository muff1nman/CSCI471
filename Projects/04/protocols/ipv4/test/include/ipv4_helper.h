...
EchoPtr google_echo_request_echo_part() {
	...
	return EchoBuilder()
		.set_request_type()
		.set_checksum(checksum) // TODO test to catch not flipv4ping of the bits
		.set_identifier(823)
		.set_sequence_num(1)
		.set_data(data)
		.build_ptr();
...
Ipv4Ptr google_echo_request_ipv4_part() {
	return Ipv4Builder()
		...
		.set_source_addr(Ipv4::SourceAddr(std::string("10001010010000111100111000011011")))
		.set_dest_addr(Ipv4::DestAddr(std::string("1001010011111011110000111010100")))
		.build_ptr();
}

RawPtr google_echo_request() {
	return RawPtr(new Raw(google_echo_request_ipv4_part(), google_echo_request_echo_part()));
}
...
Ipv4Ptr google_echo_reply_ipv4_part() {
	return Ipv4Builder()
		...
		.set_source_addr(Ipv4::SourceAddr(std::string("1001010011111011110000111010100")))
		.set_dest_addr(Ipv4::DestAddr(std::string("10001010010000111100111000011011")))
		.build_ptr();
}

RawPtr google_echo_reply() {
	return RawPtr(new Raw(google_echo_reply_ipv4_part(), google_echo_reply_echo_part()));
}
